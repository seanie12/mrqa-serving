...
def read_squad_examples(input_file, debug=False):
    ...
    for item in unproc_data:
        ...
        for token in item['context_tokens']:
            # BERT has only [SEP] in it's word piece vocabulary. because we kept all separators char length 5
    ...
    for (example_index, example) in tqdm(enumerate(examples), total=len(examples)):
        ...
        for (doc_span_index, doc_span) in enumerate(doc_spans):
            ...
            if is_training:
                ...
                # during training discard unnecessary variables for memory efficiency
                tokens = None
